/*
 *  agents/backend/backend_ai_functions.rs
 *  rust-auto-gpt-coder
 *
 *  This code is copied from:
 *  https://github.com/PacktPublishing/Craft-an-AutoGPT-Code-Generation-AI-Instrument-Leveraging-Rust-and-GPT-4
 *
 *  It was further modified by Joel Lopes Da Silva on 2/1/2026
 *  to add instructions to avoid surrounding the code with
 *  triple backticks to designate a code section.
 *
 *  It was also modified to allow the is_route_dynamic field
 *  of the REST API endpoints schema to be returned as a 
...
pub fn print_backend_webserver_code(_project_description_and_template: &str) {
    ...
    /// Do NOT print the code surrounded by triple backticks to designate a code section. ONLY print the code itself.
    println!(OUTPUT)
}
...
pub fn print_improved_webserver_code(_project_description_and_template: &str) {
    ...
    ///   3. ONLY writes the code. No commentary. No surrounding triple backticks to designatte a code section. ONLY the code.
...
pub fn print_fixed_code(_broken_code_with_bugs: &str) {
    ...
    /// IMPORTANT: Only prints out the new and improved code. No commentary or anything else. No surrounding triple backticks to designatte a code section. ONLY the code.
    println!(OUTPUT)
}
...
pub fn print_rest_api_endpoints(_code_input: &str) {
    ...
    /// MUST READ: All values are strings, with only one exception: the bool one for the key "is_route_dynamic" should not be wrapped between quotes.
    /// EXAMPLE:
    /// INPUT_CODE:
    /// ...
    /// pub struct Item {
